...
def face_swap(img,name):
    ...
    if (s == "error"):
        print ("NO or too many faces :::  abort current process ::: ")
        return img
...
filter_image =  "images/pietro.jpg" # image used to swap the original face goes here
dlibOn = False
 # We are running with Dlibon = False because we are going to using Cascade file
# to find landmarks instad of Dlib

#running an infinite loop to get frames of the inputed video
while True:
    ret, frame = cap.read()
    ...
    # flipping image for mirror like image production
    frame = cv2.flip(frame, 1)

    cv2.imshow('FaceSwapper', face_swap(frame, filter_image))
